...
void GeoPro::on_actionClear_all_triggered() {
  QMessageBox::StandardButton reply = QMessageBox::question(
      this, "DELETE ALL", "Are you sure you want to clear the board?",
      QMessageBox::Yes | QMessageBox::No);
  if (reply == QMessageBox::Yes) {
    b->getAllObj().clear();
    b->update();
...
void GeoPro::on_actionIntersection_triggered() {
  ...
  if (b->num_obj_selected > 2 ||
      (b->num_obj_selected < 2 && b->num_obj_selected > 0)) {
    QMessageBox::critical(b, "INTERSECTION ERROR",
                          "Can intersect only 2 objects!");
    b->unselectAll();
    b->update();
    return;
  }
  if (b->num_obj_selected == 0) {
    QMessageBox::warning(b, "INTERSECTION WARNING",
                         "Select 2 objects to intersect!");
    return;
  }
  if (b->num_obj_selected == 2) {
    ...
    for (auto& obj : b->getAllObj()) {
      if (obj->isSelected()) {
        intersection->parents_intersected.insert(
            std::make_pair(obj->type_is(), obj));
        if (!Obj1) {
          Obj1 = obj;
          continue;
        }
    ...
    if (Obj1 == 0 || Obj2 == 0) {  // !Obj1 || !Obj2
      ...
      QMessageBox::critical(b, "FATAL LOGIC ERROR",
                            "Null object intersection!");
      QApplication::quit();
      return;
    }
    if (Obj1->type_is() == GObj_Type::POINT ||
        Obj2->type_is() == GObj_Type::POINT) {
      delete intersection;
      delete intersection2;
      b->unselectAll();
      b->update();
      QMessageBox::critical(b, "INTERSECTION ERROR",
                            "Cannot intersect with a point!");
      return;
    }
    if (Obj1->type_is() == GObj_Type::CIRCLE ||
        Obj2->type_is() == GObj_Type::CIRCLE)
      inter_circle = true;
...
void GeoPro::on_actionCircle_by_3_points_triggered() {
  ...
  if (b->num_obj_selected > 3 ||
      (b->num_obj_selected < 3 && b->num_obj_selected > 0)) {
  ...
  if (b->num_obj_selected == 3) {
    ...
    if (p1->type_is() != GObj_Type::POINT ||
        p2->type_is() != GObj_Type::POINT ||
        p3->type_is() != GObj_Type::POINT) {
      b->unselectAll();
      b->update();
      QMessageBox::critical(b, "CIRCLE ERROR",
                            "Cannot build a circle! Need 3 points.");
      return;
    }
    auto params = getCircleCenterAndRadius(static_cast<Point*>(p1),
                                           static_cast<Point*>(p2),
                                           static_cast<Point*>(p3));
...
void GeoPro::on_actionMiddle_Center_triggered() {
  ...
  if (b->num_obj_selected > 2 ||
      (b->num_obj_selected < 2 && b->num_obj_selected > 0)) {
    QMessageBox::critical(b, "MIDDLE ERROR",
                          "Can find a middle for only 2 points!");
    b->unselectAll();
    b->update();
    return;
  }
...
void GeoPro::on_actionAbout_triggered() {
  QMessageBox::information(b, "About GeoPro",
                           "GeoPro is a professional geometry toolkit for "
                           "teachers & students made by Nick Rusfort (Nikolay "
                           "Kozakov). \nYou can find more detailed information "
                           "in the README file in Github: "
                           "https://github.com/rusfort/GeoPro");
}

void GeoPro::on_actionContact_author_triggered() {
  QMessageBox::information(b, "Contact author",
                           "Nick Rusfort (Nikolay Kozakov) \nContacts: "
...
void GeoPro::on_actionParallel_line_triggered() {
  ...
  if (b->num_obj_selected == 2) {
    ...
    if (p2->type_is() != GObj_Type::LINE && p2->type_is() != GObj_Type::RAY &&
        p2->type_is() != GObj_Type::SEGMENT) {
      b->unselectAll();
      b->update();
    ...
    if (p2->type_is() != GObj_Type::LINE) {  // making lines (if not a line)
      if (p2->type_is() == GObj_Type::RAY)
        line = new Line(b, static_cast<Ray*>(p2));
      if (p2->type_is() == GObj_Type::SEGMENT)
        line = new Line(b, static_cast<Segment*>(p2));
    } else
      line = static_cast<Line*>(p2);

    auto point1 = static_cast<Point*>(p1);
    auto parallel =
        new Line(b, line->k(), -1, (point1->Y - line->k() * point1->X));
    ...
    if (p1->type_is() != GObj_Type::POINT) {  // swap if the order is not ok
      if (p2->type_is() != GObj_Type::POINT) {
        ...
        QMessageBox::critical(
            b, "LINE ERROR", "Cannot build a perpendicular line! Need a point.");
        return;
      } else {
        GOBJ* tmp = p1;
        p1 = p2;
...
void GeoPro::on_actionBisector_triggered() {
  ...
  if (b->num_obj_selected == 3) {
    ...
    QPointF res1 =
        QPointF(static_cast<Point*>(p1)->X, static_cast<Point*>(p1)->Y);
    QPointF res2 =
        QPointF(static_cast<Point*>(p2)->X, static_cast<Point*>(p2)->Y);
    QPointF res3 =
        QPointF(static_cast<Point*>(p3)->X, static_cast<Point*>(p3)->Y);
